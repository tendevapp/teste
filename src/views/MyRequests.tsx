...
import { 
  ...
  Calendar, Check, AlertCircle, Copy, CheckCircle2, ChevronRight, Download, Upload, Loader2, Info
} from 'lucide-react';
...
export default function MyRequests({ user }: MyRequestsProps) {
  ...
  useEffect(() => {
    // Load requests
    const list = localDb.getRequests().filter(r => 
  ...
  const handlePostComment = (e: React.FormEvent) => {
    ...
    // Refresh list/data
    setRequests(localDb.getRequests().filter(r => 
      r.solicitante_id === user.id || 
      (user.roles.includes('gestor') && r.solicitante_sector_id === user.sector_id) ||
      user.roles.includes('admin') ||
      user.roles.includes('coordenador_suprimentos') ||
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-6">
              ...
              <div className="rounded-xl bg-white p-5 border border-slate-100 space-y-3">
                <div className="flex items-center justify-between">
                  <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Anexos ({selectedRequest.attachments?.length || 0})</h4>
                  <label className="text-xs font-bold text-emerald-700 hover:text-emerald-900 cursor-pointer inline-flex items-center">
                    <Upload className="h-3.5 w-3.5 mr-1" />
                    <span>Upload</span>
                    <input type="file" onChange={handleSimulateAttachmentUpload} className="hidden" />
                  </label>
                </div>

                {uploadingFile && (
                  <div className="text-xs text-slate-500 flex items-center py-2"><Loader2 className="animate-spin h-3.5 w-3.5 mr-2" /> Carregando arquivo...</div>
                )}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {selectedRequest.attachments.map((file, idx) => (
                      <div key={idx} className="flex items-center justify-between bg-slate-50 p-2 rounded border border-slate-100 text-xs">
                        <span className="truncate max-w-[150px] font-medium text-slate-700">{file}</span>
                        <button className="text-slate-400 hover:text-slate-600" title="Simular Download">
                    ...
                    comments.map((comm) => {
                      ...
                      return (
                        <div key={comm.id} className={`flex flex-col text-xs max-w-[85%] ${isMe ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                          ...
                          <div className={`p-3 rounded-xl border ${isMe ? 'bg-emerald-800 text-white border-emerald-800 rounded-br-none' : 'bg-slate-100 text-slate-800 border-slate-100 rounded-bl-none'}`}>
                            <p>{comm.content}</p>
                          </div>
                        </div>
                      );
